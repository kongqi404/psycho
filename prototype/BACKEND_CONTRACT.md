...
8. **随手闪念默认留在本机**：独立便签及其压缩图片不是后端画像数据源。只有用户主动选择「用它开始梳理」后，客户端才可把单张便签文字复制进当前章节；不得后台批量上传、识图、向量化或同步全部便签与图片。
...
| 随手闪念及压缩预览图 | 当前原型仅本机，最多 50 张，保留至用户删除 | 单张或全部删除；主动选择后只复制一张便签的文字到当前章节 |
...
删除任务返回 `deletion_job_id`、范围和预计完成时间。撤回个性化或训练授权后，新的请求立即停止相关用途；历史数据按对应删除流程清除。服务端删除不能代替本机 `localStorage` 清理，客户端必须同时提供清空随手闪念、卡槽和未来回响的入口。
