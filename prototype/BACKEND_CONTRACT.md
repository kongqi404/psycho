...
8. **闪念提交即收录**：用户首次使用统一闪念输入栏时明确看到“提交后用于初印象、日报与个性化内容”的用途说明。之后提交的文字、图片和语音自动保存到账户；文字及语音转写可以生成可解释的低粒度主题信号。图片默认只保存和展示，不自动识图。原始内容、派生信号、人工质检和模型训练仍必须分层授权与删除。
9. **语音默认不启动采集**：当前语音输入与语音对话只做前端状态演示。正式接入前必须在用户主动点击后再请求麦克风权限，并明确展示录音、上传、转写、保存和删除状态。
...
`POST /v1/thoughts` 在一次请求中接收文字和已经上传完成的图片、语音附件：

```json
{
  "client_thought_id": "thought_client_01",
  "text": "今天明明很累，还是不敢停下来。",
  "attachments": [
    { "asset_id": "asset_photo_01", "type": "image" },
    { "asset_id": "asset_voice_01", "type": "audio", "duration_ms": 12000 }
  ],
  "personalization_notice_version": "thoughts-v1"
}
```

成功响应返回 `thought_id`、`saved_at`、附件处理状态，以及 `profile_update.status`（`pending | applied | skipped`）。客户端提交成功后立即把闪念加入卡片墙；派生信号异步完成，不应阻塞保存。语音转写失败不影响原始语音保存，图片未开启识图时 `profile_update` 不得根据图片内容生成信号。

`GET /v1/thoughts` 分页返回卡片墙；`DELETE /v1/thoughts/{thought_id}` 删除单条原文、附件、转写和由该条内容产生的派生信号。所有创建请求必须支持客户端幂等 ID，避免弱网重试产生重复卡片。
...
```json
{
  ...
  "basis": [
    ...
    {"code": "grounding_cards_more_frequent", "label": "安定潮笺出现较多"}
  ],
...
| 随手闪念、附件、转写与来源映射 | 账户内保存至用户删除；派生主题使用滚动 30 天窗口，过期后不再参与日报 | 查看、导出、单条或全部删除、关闭后续个性化；删除来源时同步删除派生信号 |
| 答案之书 | 当前原型仅本机保存当天日期与内置卡片序号 | 次日自动失效；清理网站数据 |
...
- `DELETE /v1/thoughts/{thought_id}`：删除单条闪念原文、附件、转写及来源派生信号。
- `DELETE /v1/thoughts`：删除全部闪念及其派生信号。
...
删除任务返回 `deletion_job_id`、范围和预计完成时间。撤回个性化或训练授权后，新的请求立即停止相关用途；历史数据按对应删除流程清除。闪念删除必须同时覆盖账户主记录、附件、转写和来源派生信号；客户端在服务端确认后同步移除卡片。
